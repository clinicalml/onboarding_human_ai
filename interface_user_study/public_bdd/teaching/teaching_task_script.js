...
function createAddContainer(photoID, rawCaption, i) {
  ...
  addCaption(container.id, rawCaption);
...
function addPhoto(containerID, photoID) {
  ...
  var imageRef = storage.ref("/bdd_images/" + photoID);
  // Get the download URL of the image
  imageRef
    .getDownloadURL()
    .then(function (url) {
      // Set the URL as the source of the img element to display the image
      photo.src = url; // placeholder code--will be replaced with actual photos in BDD file
    })
    .catch(function (error) {
      // Handle any errors
      console.error(error);
 ...
 * helper to createAddContainer
 */
function addCaption(containerID, rawCaption) {
  const container = document.getElementById(containerID);
  const captionSpan = document.createElement("span");
  captionSpan.className = "caption";
  const captionText = document.createTextNode(rawCaption); // placeholder code--will be replaced with actual captions in BDD file
